...
from .models import Booking, BookingStatus, Nomer, StatusNomera, User
...
def format_booking_period_rus(
    start_date, end_date, time_start="14:00", time_end="12:00"
):
    ...
    try:
        ...
        start_str = (
            f"{dt_start.day} {months[dt_start.month]} {dt_start.year} {time_start}"
        )
...
async def login(
    ...
    if client_ip in failed_login_attempts:
        if failed_login_attempts[client_ip]["block_until"] > now:
            remaining = int(
                (failed_login_attempts[client_ip]["block_until"] - now).total_seconds()
            )
    ...
    if not user or not verify_password(password, user.hashed_password):
        ...
        if failed_login_attempts[client_ip]["attempts"] >= 5:
            failed_login_attempts[client_ip]["block_until"] = now + timedelta(
                minutes=10
            )
            return templates.TemplateResponse(
                "login.html",
...
async def dashboard(
    ...
    if filter == "clean":
        ...
        filter == "broken"
    ):  # Assuming user meant "repair" for code but filter string was checked differently in previous attempts
    ...
    bookings_res = await db.execute(
        select(Booking)
        .where(Booking.status != BookingStatus.CANCELLED, Booking.end_date >= today_str)
    ...
    for nomer in nomera:
        ...
        if bookings:
            ...
            if nearest_booking:
                ...
                if nearest_booking.start_date <= today_str <= nearest_booking.end_date:
                    ...
                    nomer.status = (
                        StatusNomera.ZANYATO
                        if hasattr(StatusNomera, "ZANYATO")
                        else nomer.status
                    )
        ...
        if filter == "reserved":
            ...
            if nomer.gotov_k_sdache and not nomer.zanyat:
                filtered_nomera.append(nomer)
        else:
            filtered_nomera.append(nomer)
...
async def update_nomer(
    ...
    old_otvetstvenniy = nomer.otvetstvenniy
    ...
    if status:
        ...
        if status in [
            StatusNomera.CHISTO,
            StatusNomera.REMONT,
            StatusNomera.TREBUET_UBORKI,
        ]:
            nomer.otvetstvenniy = None
    if otvetstvenniy is not None:
        nomer.otvetstvenniy = otvetstvenniy

    # Уведомления Telegram
    if nomer.otvetstvenniy != old_otvetstvenniy:
        # 1. Если был ответственный, уведомляем о снятии
        if old_otvetstvenniy:
            asyncio.create_task(
                bot_service.notify_removal(
                    user_identifier=old_otvetstvenniy,
                    room_number=nomer.nomer_komnati
                )
            )
        
        # 2. Если назначен новый, уведомляем о назначении
        if nomer.otvetstvenniy:
            task_type = "cleaning"
            if nomer.status == StatusNomera.REMONT:
                task_type = "repair"
            
            # Уведомления сотруднику
            asyncio.create_task(
                bot_service.notify_assignment(
                    user_identifier=nomer.otvetstvenniy,
                    room_number=nomer.nomer_komnati,
                    room_id=nomer.id,
                    task_type=task_type
                )
        ...
        # 3. Логирование для администраторов
        log_text = (
            f"🔔 <b>ЛОГ: ИЗМЕНЕНИЕ НАЗНАЧЕНИЯ</b>\n"
            f"🏨 Номер: {nomer.nomer_komnati}\n"
            f"👤 Старый ответственный: {old_otvetstvenniy or 'Нет'}\n"
            f"👤 Новый ответственный: {nomer.otvetstvenniy or 'Нет'}\n"
            f"👨‍💻 Изменил: {user.full_name or user.username}"
        )
        asyncio.create_task(bot_service.notify_admins(log_text))
    ...
    if fio_zhilca is not None:
        nomer.fio_zhilca = fio_zhilca
...
async def guest_details(
    ...
    if date:
        query = query.where(Booking.start_date <= date, Booking.end_date >= date)
    else:
        query = query.where(Booking.end_date >= today_str)

    bookings_res = await db.execute(query.order_by(Booking.start_date))
    booking = bookings_res.scalars().first()

    if booking:
        nomer.fio_zhilca = (
            booking.guest_name
        )  # Use booking name as resident name for display
    ...
    return templates.TemplateResponse(
        "partials/guest_details.html",
        {"request": request, "nomer": nomer, "show_back": back},
    )
...
async def cancel_reserve(
    ...
    if (
        hx_target == "guest-details-modal"
        or hx_target == "closest .modal-overlay"
        or "calendar" in referer
    ):
        return HTMLResponse("<script>window.location.reload();</script>")
...
async def staff_list(
    ...
    return templates.TemplateResponse(
        "staff_management.html", {"request": request, "staff": staff, "user": user}
    )
...
async def add_staff(
    ...
    return templates.TemplateResponse(
        "partials/staff_row.html", {"request": request, "s": new_user}
    )
...
async def staff_details_modal(
    ...
    return templates.TemplateResponse(
        "partials/staff_details_modal.html", {"request": request, "s": target_user}
    )
...
async def update_staff(
    ...
    return templates.TemplateResponse(
        "partials/staff_row.html", {"request": request, "s": target_user}
    )
...
async def occupancy_calendar(
    ...
    # Function to generate color from ID
    def get_color(id):
        hue = (id * 137.508) % 360
    ...
    for nomer in nomera:
        ...
        for day in days:
            ...
            day_data = {"status": 0, "booking_id": None, "color": None}
            ...
            for b in nomer_bookings:
                if b.start_date <= day_str <= b.end_date:
                    ...
                    day_data["booking_id"] = b.id
                    day_data["color"] = get_color(b.id)
                    break

            days_status.append(day_data)

        calendar_data.append({"nomer": nomer, "days": days_status})
...
async def assign_staff_modal(
    ...
    return templates.TemplateResponse(
        "partials/assign_staff_modal.html",
        {
            "request": request,
            "nomer": nomer,
